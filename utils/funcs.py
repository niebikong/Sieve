...
def knn_predict(feature, feature_bank, feature_labels, classes, knn_k, batch_size=1000):
    ...
    if num_features * num_bank > 100000000:  # ~400MB for float32
        ...
        for i in range(0, num_features, batch_size):
            ...
            batch_feature = feature[i:end_idx]
            ...
            all_sim_indices.append(sim_indices_batch)
            
            # Clear cache
            del sim_matrix_batch
            torch.cuda.empty_cache()
        
        sim_weight = torch.cat(all_sim_weights, dim=0)
        sim_indices = torch.cat(all_sim_indices, dim=0)
    else:
