...
def pairwise_distance(data1, data2, batch_size=None):
    """
    Calculate pairwise Euclidean distance using broadcast mechanism.
...
class K_Means:
    ...
    Supports both standard K-Means and semi-supervised K-Means where
    some samples have known labels.
    """
    ...
    def fit_mix_once(self, u_feats, l_feats, l_targets, random_state):
        """
        Single run of semi-supervised K-Means.
        
        Args:
            u_feats: unlabeled features
            l_feats: labeled features
            l_targets: labels for labeled features
            random_state: random state for reproducibility
        
        Returns:
            labels, inertia, centers, n_iterations
        """
        def supp_idxs(c):
        ...
        l_targets_np = l_targets.cpu().long().numpy()
        l_num = len(l_targets_np)
        cid2ncid = {cid: ncid for ncid, cid in enumerate(l_classes)}
        for i in range(l_num):
            labels[i] = cid2ncid[l_targets_np[i]]
        ...
        return best_labels, best_inertia, best_centers, it + 1
    ...
    def fit(self, X):
        ...
        if effective_n_jobs(self.n_jobs) == 1:
            ...
            results = Parallel(n_jobs=self.n_jobs, verbose=0)(
                delayed(self.fit_once)(X, seed) for seed in seeds
            )
